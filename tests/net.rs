...
use turmoil::{debug, hold, lookup, net, Builder};
...
#[test]
fn simple_connect() -> turmoil::Result {
    let mut sim = Builder::new().build();

    sim.client("server", async move {
        let listener = net::TcpListener::bind((UNSPECIFIED, 1738)).await?;
    ...
    sim.client("client", async move {
        let addr: SocketAddr = (lookup("server").ip(), 1738).into();
        let _ = net::TcpStream::connect(addr).await?;
...
fn network_partitions() -> turmoil::Result {
    ...
    sim.host("server", || async {
        let listener = net::TcpListener::bind((UNSPECIFIED, 1738)).await.unwrap();
        loop {
            let _ = listener.accept().await;
        }
    });
    ...
    sim.client("client", async {
        ...
        assert!(
            timeout(Duration::from_secs(1), net::TcpStream::connect(addr))
                .await
                .is_err()
        );
...
fn accept_front_of_line_blocking() -> turmoil::Result {
    ...
    sim.client("A", async move {
        ...
        let addr: SocketAddr = (lookup("B").ip(), 1738).into();
        assert!(
            timeout(Duration::from_secs(1), net::TcpStream::connect(addr))
                .await
                .is_err()
